...
- The judge never sees the original image, audio, or file content
- If evaluation needs to reason about the multimodal input (e.g., "did the agent correctly describe this image?"), the built-in judge cannot verify it
- A custom metric is required for true multimodal evaluation

---

## Custom Metric for Multimodal Evaluation

Custom metric functions receive full `Invocation` objects including all multimodal parts. Build a prompt that sends the original media plus the agent response to a vision-capable judge model.
...
```json
{
  ...
  "custom_metrics": {
    "multimodal_response_quality": {
      ...
      "description": "Evaluates response accuracy against multimodal input using vision-capable judge"
    }
  }
}
```
