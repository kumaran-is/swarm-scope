...
Static `conversation` is better when: the interaction is fully deterministic, you need trajectory matching, or you need reference answers for `response_match_score` / `final_response_match_v2`.
...
- `starting_prompt` — fixed first user message sent verbatim to the agent
...
Reference-data criteria (`tool_trajectory_avg_score`, `response_match_score`, `final_response_match_v2`) require ground-truth answers and are incompatible with user simulation — ground truth cannot be known when responses are generated dynamically.

| Criterion | Compatible with User Simulation |
|-----------|:-------------------------------:|
...
- Fetch `https://google.github.io/adk-docs/evaluate/user-sim/index.md` for the full user simulation guide with additional examples.
