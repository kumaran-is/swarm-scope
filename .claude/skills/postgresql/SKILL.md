...
context: fork
metadata:
  triggers: PostgreSQL schema, PostgreSQL gotchas, MVCC, HOT update, fillfactor, UNLOGGED table, TOAST storage, EXCLUDE constraint, PostgreSQL enum, array type, range type, network type, domain type, composite type, generated column, pgcrypto, pg_trgm, timescaledb, postgis, pgvector extension, JSONB GIN, jsonb_path_ops, safe schema evolution, volatile default, CREATE INDEX CONCURRENTLY, UUIDv7, insert-heavy, upsert design
  related-skills: database-schema-designer, postgres-best-practices, sql-optimization-patterns, vector-database, sql-pro
